#' Size of joint assignment grid
#'
#' Calculates how many rows would be produced by [expand.grid.nodup()], either exactly
#' or as a guaranteed lower bound. This is used to detect DVI problems where a joint 
#' search would be prohibitive.
#' 
#' Exact counting is done without constructing the full table. The algorithm counts 
#' matchings in the bipartite graph between victims and missing persons, treating the
#' non-assignment value "*" as a repeatable choice. For large problems, the function 
#' returns a lower bound instead of the exact count. (See Value.)
#'
#' @param dvi A `dviData` object. If `pairings` is not supplied, candidate
#'   pairings are taken from `dvi$pairings`, or generated with
#'   [generatePairings()].
#' @param pairings A list of character vectors, typically produced by
#'   [generatePairings()]. If supplied, `dvi` is ignored.
#' @param ignoreSex A logical, passed on to [generatePairings()] if pairings are
#'   generated from `dvi`.
#' @param max A positive number. Exact counting may stop once the count is known
#'   to exceed this value. Default: `Inf`.
#'
#' @return A list with elements `size` (a number) and `type` (either
#'   "exact" or "lower bound").
#'
#' @seealso [expand.grid.nodup()], [dviJoint()], [generatePairings()], [ncomb()]
#'
#' @examples
#' dviGridSize(example1)
#'
#' pairings = generatePairings(example1)
#' dviGridSize(pairings = pairings)
#'
#' @export
dviGridSize = function(dvi = NULL, pairings = NULL, ignoreSex = FALSE, max = Inf) {
  if(is.null(pairings)) {
    if(is.null(dvi))
      stop2("Either `dvi` or `pairings` must be supplied")
  ...
  b = .gridSizeExact(pairings, cap = 10001)
  if(b <= 10000) 
    return(list(size = b, type = "exact"))
  
  b = .gridSizeLowerBound(pairings)
  if(b > max) 
    return(list(size = b, type = "lower bound"))
  
  b = .gridSizeExact(pairings, cap = max + 1)
  if(b < max) 
    return(list(size = b, type = "exact"))
  else 
    return(list(size = b, type = "lower bound"))
...
.gridSizeExact = function(lst, cap = Inf) {
  ...
  if(m > 53L) {
    warning("Too many shared values; exact counting not supported", call. = FALSE)
    return(cap)
  }
