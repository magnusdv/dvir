...
#' pairings and detection of undisputed matches) and performs the final joint analysis. As
#' the package evolved, it became beneficial to split these two roles, resulting in the
#' two functions above.
#'
...
#' @param limit A positive number, by default 0. Only pairwise LR values above this are
#'   considered.
#' @param nkeep An integer, or NULL. If given, only the `nkeep` most likely pairings are
#'   considered for each victim.
#' @param markers A vector indicating which markers should be included in the analysis. By
#'   default all markers are included.
#' @param disableMutations A logical, or NA (default). The default action is to disable
#'   mutations in all reference families without Mendelian errors.
...
#'   Default: 1e5.
#' @param threshold A positive number, passed onto [findUndisputed()]. Default: 1e4.
...
#' @param LRthresh A positive number, used as upper limit for the LR comparing the top
#'   result with all others.
#'
#' @return A data frame. Each row describes an assignment of victims to missing persons,
#'   accompanied with its log likelihood, the LR compared to the null (i.e., no
#'   identifications), and the posterior corresponding to a flat prior.
#'
#'   The function `compactJointRes()` removes columns without assignments, and solutions
#'   whose LR compared with the top result is below `1/LRthresh`.
#'
#' @references Vigeland, MD., Egeland, T. Joint DNA-based disaster victim identification.
