...
#' information. 
#'
#' @param famfile Name of Familias file.
#' @param missingPrefix. 
#' @param verbose A logical. Passed onto [readFam()]
#'
#' @return A `dviData` object. The missing persons are renamed. The first
#' number indicates the family, the second the missing person in the family.
#' 
#' @details The sex of the missing persons need to be checked as this information
#' may not be correctly recorded in the fam file.
#'
...
familiasTodvir = function(famfile, missingPrefix = "M", verbose = FALSE){
  ...
  x = gsub("\\\"", "", raw)
  if (!"[DVI]" %in% x)
    stop2("No DVI input found. Use forell::readFam to read the file.")
  
  x = readFam(famfile, verbose = verbose)

  if(length(x) == 1) # There are no reference families, no information on the missing.
    stop2("No reference families found. Use forell::readFam to read the file.")
