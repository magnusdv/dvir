...
```{r, example, echo = FALSE, fig.height = 3, fig.width = 5, out.width = "50%"}
library(dvir)
library(pedtools)
pm = example2$pm
am = example2$am
...
_a_ to the null model., i.e., LR=_L(a)/L0_ . 
...
_P(Vi = Mj_ | data) and _P(Vi =_ '*' | data) for _i, j = 1, 2, 3_. 
...
The data are available within the library  and can be extracted as follows
